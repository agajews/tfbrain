...
class AsyncSIACAgent(object):
    ...
    def build(self):
        ...
        # self.advantage_clip = self.hyperparams['advantage_clip']
        ...
        # log_preds = tf.log(self.ac_model.policy_y_hat)
        self.entropy = -tf.reduce_sum(
            self.ac_model.policy_y_hat * log_preds,
            reduction_indices=1)
        # self.log_preds = tf.reduce_sum(log_preds * self.mask,
        #                                reduction_indices=1)
        # preds = tf.reduce_sum(self.ac_model.policy_y_hat * self.mask,
        #                       reduction_indices=1)
        preds = tf.reduce_sum(log_preds * self.mask,
                              reduction_indices=1)
        # preds = tf.tanh(preds * self.hyperparams['pred_scale'])
        self.mean_entropy = tf.reduce_mean(self.entropy)
        entropy_scale = self.hyperparams['entropy_scale']
        # self.advantage = self.returns - self.pred_value
        # self.advantage = tf.clip_by_value(
        #     self.advantage, -self.advantage_clip, self.advantage_clip)
        self.policy_loss = tf.reduce_mean(preds * self.advantage +
                                          self.entropy * entropy_scale)
        self.value_loss = -tf.reduce_mean(
            tf.square(self.returns - self.ac_model.value_y_hat)) / 2
    ...
    def aggregate_from(self, chooser):
        ...
        for episode_cache in chooser.experience_cache:
            episode_value = 0
            for batch_num in reversed(range(0, len(episode_cache), tmax)):
                batch = episode_cache[batch_num:batch_num + tmax]
                batch_value = 0
                ...
                for experience_num, experience in enumerate(
                    ...
                    pred_value = self.ac_model.compute_value_preds(
                        {'state': np.expand_dims(state, 0)}, sess=self.sess)
                    experience = (state, action, reward, last_state,
                                  episode_value, batch_value,
                                  pred_value, last_state_value,
                                  experience_num)
                    self.experience_replay.append(experience)
                    batch_value = reward + reward_discount * batch_value
                    episode_value = reward + reward_discount * episode_value
        chooser.experience_cache = []
    ...
    def batch_preprocessor(self, batch):
        ...
        train_xs = {'state': np.array([s for (s, a, r, n, e, b, p, l, x)
                                       in batch])}
        ...
        for experience_num, (state, action, reward, next_state,
                             ...
                             num) in enumerate(batch):
            returns = reward + reward_discount * episode_value
            # target = returns - pred_value
            # train_advantage[experience_num] = target
            n_returns = (reward + reward_discount * batch_value +
                         reward_discount ** (num + 1) * last_value[0, 0])
            train_advantage[experience_num] = n_returns - pred_value[0, 0]
        ...
        self.advantage_val = n_returns - pred_value[0, 0]
    ...
    def train_model(self,
        ...
        if display:
            ...
            print('Advantage: %s' % str(self.advantage_val))
            print('N returns: %s' % str(self.n_returns))
            print('Batch value: %f' % self.batch_value)
            # print('Log preds: %s' % str(log_preds))
