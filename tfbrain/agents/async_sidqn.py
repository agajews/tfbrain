...
from tfbrain.helpers import get_all_params, \
    set_all_params_ops, flatten_params
...
class AsyncSIDQNAgent(object):
    ...
    def build(self):
        ...
        tvars = flatten_params(get_all_params(self.q_model.get_net()))
    ...
    def train_model(self,
        ...
        if display:
            ...
            print('Loss: %f' % loss)
            avg_recent_loss.append(loss)
            print('Avg recent loss: %f' % (sum(avg_recent_loss) /
                                           len(avg_recent_loss)))
