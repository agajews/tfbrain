...
class AsyncEdistSNDQNV2Agent(object):
    ...
    def aggregate_from(self, chooser):
        ...
        for episode_cache in chooser.experience_cache:
            for batch_num in range(
                ...
                value = 0
                last_state = batch[-1][3]
                last_state_value = self.q_model.compute_preds(
                    {'state': np.expand_dims(last_state, 0)}, sess=self.sess)
                for experience_num, experience in enumerate(reversed(batch)):
                    state, action, reward, next_state = experience
                    experience = (state, action, reward, last_state, value,
                                  last_state_value, experience_num)
    ...
    def batch_preprocessor(self, batch):
        ...
        train_xs = {'state': np.array([s for (s, a, r, n, v, l, e) in batch])}
        train_y = np.zeros((len(batch),))
        train_mask = np.zeros((len(batch), len(self.actions)))
        # preds = self.q_model.compute_target_preds(
        #     {'state': np.array([n for (s, a, r, n, v, p, l, e) in batch])},
        #     sess=self.sess)
        for experience_num, (state, action, reward, next_state,
                             value, last_value,
                             sub_num) in enumerate(batch):
            exp_returns = np.max(last_value)
            target = (reward + reward_discount * value +
                      (reward_discount ** (sub_num + 1) * exp_returns))
