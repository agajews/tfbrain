...
class Trainer(object):
    ...
    def build_loss(self):
        self.loss_val = self.loss.build(self.model.y_hat, self.y)
...
class UnsupervisedTrainer(Trainer):
    ...
    def build_loss(self):
        self.loss_val = self.loss.build(self.model.y_hat)

    def build_stats(self):
        self.accuracy = self.accuracy_obj.build(self.model.y_hat)
    ...
    def compute_train_stats(self, batch):
        return self.compute_accuracy(batch)

    def compute_val_stats(self, val_xs):
        return self.compute_accuracy(val_xs)

    def eval(self, val_xs):
        val_stats = self.compute_val_stats(val_xs)
        self.display_val_stats(val_stats)

    def create_minibatch_iterator(self, train_xs):
        inputs = {}
        inputs.update(train_xs)
        return iterate_minibatches(
            inputs, batch_size=self.hyperparams['batch_size'])

    def train(self, train_xs, val_xs,
              display_interval=100, cmp_val_stats=False):
        '''train_xs: a dictionary of strings -> np arrays
        matching the model's input_vars dictionary
        val_xs: same as train_xs but for validation'''
        ...
        minibatches = self.create_minibatch_iterator(train_xs)
        epoch = 0
        for update in range(self.hyperparams['num_updates']):
            try:
                batch = next(minibatches)
